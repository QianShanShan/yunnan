...
import json
import re
class GgzyYnGovSpider(scrapy.Spider):
    name = "ggzy.yn.gov"
    ...
    def parse(self, response):
        ...
        for da in data_list:
            ...
            item['guid'] = guid
            # 获取标题
            item['title'] = da['tenderProjectName']
            # 获取时间
            item['publishTime'] = da['publishTime']
            yield scrapy.Request(url=f'https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getTenserPlanDetail?guid={guid}',headers=self.headers,callback=self.sec_parse,meta={'item':item})
    def sec_parse(self, response):
        item = response.meta['item']
        data = response.json().get('value')
        # 获取内容
        content = ''.join(re.findall('>(.*?)<',data['content']))
