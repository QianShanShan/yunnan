...
import json
import re
class GetcontractlistSpider(scrapy.Spider):
    name = "getContractList"
    allowed_domains = ["ggzy.yn.gov.cn"]
    start_urls = ["https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getContractList"]
    ...
    def parse(self, response):
        ...
        for da in data_list:
            ...
            item['guid'] = guid
            # 获取标题
            item['title'] = da['contractName']
            # 获取时间
            item['publishTime'] = da['gongshiTime']
            yield scrapy.Request(
                url=f'https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getGcjsContractDetail?guid={guid}',
    ...
    def sec_parse(self, response):
        ...
        # 获取内容
        content = ''.join(re.findall('>(.*?)<', data['contractContent']))
