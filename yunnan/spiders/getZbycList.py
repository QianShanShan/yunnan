...
import json
import re
class GetzbyclistSpider(scrapy.Spider):
    name = "getZbycList"
    allowed_domains = ["ggzy.yn.gov.cn"]
    start_urls = ["https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getZbycList"]
    ...
    def parse(self, response):
        ...
        for da in data_list:
            ...
            item['guid'] = guid
            # 获取标题
            item['title'] = da['exceptionName']
            # 获取时间
            item['publishTime'] = da['dateTimeStamp']
            yield scrapy.Request(
                url=f'https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/findZbycByGuid?guid={guid}',
    ...
    def sec_parse(self, response):
        ...
        # 获取内容
        content = ''.join(re.findall('>(.*?)<', data['exceptionInfor']))
