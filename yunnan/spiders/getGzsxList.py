...
import json
import re
class GetgzsxlistSpider(scrapy.Spider):
    name = "getGzsxList"
    allowed_domains = ["ggzy.yn.gov.cn"]
    start_urls = ["https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getGzsxList"]
    ...
    def parse(self, response):
        ...
        for da in data_list:
            ...
            item['guid'] = guid
            # 获取标题
            item['title'] = da['changetitle']
            # 获取时间
            item['publishTime'] = da['fabutime']
            yield scrapy.Request(
                url=f'https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/findGzsxByGuid?guid={guid}',
    ...
    def sec_parse(self, response):
        ...
        # 获取内容
        content = ''.join(re.findall('>(.*?)<', data['changecontent']))
