...
import json
import re
class GetzbjggglistSpider(scrapy.Spider):
    name = "getZbJgGgList"
    allowed_domains = ["ggzy.yn.gov.cn"]
    start_urls = ["https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getZbJgGgList"]
    ...
    def parse(self, response):
        ...
        for da in data_list:
            ...
            item['guid'] = guid
            # 获取标题
            item['title'] = da['bulletinname']
            # 获取时间
            item['publishTime'] = da['bulletinissuetime']
            yield scrapy.Request(
                url=f'https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/findZbJgGgByGuid?guid={guid}',
    ...
    def sec_parse(self, response):
        ...
        # 获取内容
        content = ''.join(re.findall('>(.*?)<', data['bulletincontent']))
